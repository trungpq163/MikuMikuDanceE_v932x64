...
VS_OUTPUT ParticleFunc(int num,float4 Pos,float deflen,float maxlen,float maxheight,float spd)
{
  VS_OUTPUT Out;
   ...
   float local_t = (time_0_X * AnmSpd + MaterialDiffuse.a + fi*2) % 1.0;
...
VS_OUTPUT ParticleVS(float4 Pos: POSITION){
   ...
   if((1-((time_0_X * AnmSpd + MaterialDiffuse.a) % 1.0)) != 0 || (1-((time_0_X * AnmSpd + MaterialDiffuse.a) % 1.0)) != 1)
   {
   		Out = ParticleFunc(ParticleNum,Pos,0.5,0.75,0,0.2);
   }
   return Out;
}
VS_OUTPUT Particle2VS(float4 Pos: POSITION){
   VS_OUTPUT Out = (VS_OUTPUT)0;
   if((1-((time_0_X + MaterialDiffuse.a) % 1.0)) != 0 || (1-((time_0_X + MaterialDiffuse.a) % 1.0)) != 1)
   {
...
float4 AllPS(float2 texCoord: TEXCOORD0, float color: TEXCOORD1) : COLOR {
   ...
   col.a *= color * DefAlpha * MaterialDiffuse.a;
   return col;
}
//合成方法の設定
//
