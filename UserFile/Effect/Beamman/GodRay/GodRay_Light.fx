...
VS_OUTPUT VS_BufferRender( float4 Pos : POSITION, float4 Tex : TEXCOORD0 ){
    ...
    Out.Tex = Tex;
    float4 target = mul(float4(normalize(-LightDirection)*1000,1),mVP);
...
{
	LightSize = 1-LightSize;
	LightBoke = 1-LightBoke;
	float4 Color;
	Color = tex2D( B2Samp, Tex);
	Color *= (1-(pow(length(Tex - Center)*(LightSize),1+LightBoke*5.0)-1));
...
float4  PS_Blur(float2 Tex: TEXCOORD0 ,float2 Center: TEXCOORD1,uniform sampler2D Samp,uniform bool Cpy) : COLOR0
{
	...
	if(!Cpy)
	{
		...
		dir *= 0.5+LightLen*length(WorldMatrix[0])*0.1;
		//距離を計算する
