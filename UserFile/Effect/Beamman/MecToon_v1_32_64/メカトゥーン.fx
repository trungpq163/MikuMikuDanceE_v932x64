...
struct BufferShadow_OUTPUT {
	...
	float4 ScreenTex : TEXCOORD8;   // スクリーン座標
};
...
{
	if(!useTexture) return float4(Normal,1);
	float4 Norm = 1;
...
inline float3 CalcSpecular(float3 L,float3 N,float3 V,float3 Col)
{
	...
	float last = max(0, F*D*G/NV);
	last = smoothstep(0.8,0.9,last);
...
float4 BufferShadow_Mec_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
	...
	float MainD = dot( normal, -LightDirection )*half_mul+half_add;
	float BackD = dot( normal, normalize(normalize(-Eye) ) )*half_mul+half_add;
	float FillD = dot( normal, normalize(Eye) )*half_mul+half_add;
	
	MainD = smoothstep(0.4,0.6,MainD);
	BackD = smoothstep(0.4,0.6,BackD);
	FillD = smoothstep(0.4,0.6,FillD);
	
	Color.rgb = max(0,MainD * AmbientColor * LightAmbient * 2);
	Color.rgb += max(0,BackD * AmbientColor * BackLight*2* LightAmbient * 2);
	Color.rgb += max(0,FillD * AmbientColor * FillLight*0.25* LightAmbient * 2);
    ...
    ;
    //簡易リムライト
    float rim = 1-saturate(dot(V,normalize(IN.Normal)));
    
    rim = smoothstep(0.5,1,rim);
    //return float4(rim,0,0,1);
    Specular += rim * LightAmbient*2;
    ...
    Mirror_Param *= saturate(length(Specular));
    ...
    if ( useTexture ) {
	    ...
	    TexColor.rgb += lerp(0,texDP(sampEnvMapF, sampEnvMapB, reflVec.xyz),Mirror_Param).rgb;
        Color *= TexColor;
    ...
    //高さマップによる色補正
    Color.rgb *= saturate(height+0.7);
    ShadowColor.rgb *= saturate(height+0.7);
    ...
    if(Exist_ExcellentShadow){
        ...
        SadowMapVal = smoothstep(0.4,0.6,SadowMapVal);
        
        Color = lerp(ShadowColor, Color, SadowMapVal);
        Color.rgb = pow(Color.rgb+0.1,1.5);
        return Color;
