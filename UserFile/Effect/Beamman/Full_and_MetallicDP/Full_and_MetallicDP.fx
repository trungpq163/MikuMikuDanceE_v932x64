//full.fxとMetallicDP.fxをただ合成したもの
//ビームマンP

//パラメータ
//全体の色（乗算）
float4 BodyColor = float4(1.5,1.5,1.5,1.5);
...
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR0
{
    ...
    return texDP(sampEnvMapF, sampEnvMapB, reflVec.xyz)*Color*BodyColor;
    //return Color;
}
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    if( any( saturate(TransTexCoord) != TransTexCoord ) ) {
		...
		float3 reflVec = reflect(-normalize(IN.Eye), normalize(IN.Normal));
		return texDP(sampEnvMapF, sampEnvMapB, reflVec.xyz)*ans*BodyColor;
        //return ans;
    }
}
