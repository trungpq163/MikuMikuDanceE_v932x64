...
static float2 MirrorSize = { 500*saturate(morph_width-0.1), 500*saturate(morph_width-0.1) };

float3   CameraPosition    : POSITION  < string Object = "Camera"; >;
// 普通のビュー行列
float4x4 calcViewMatrixInUp(float4x4 matWorld) {

    float3 eye = matWorld[3].xyz+normalize(matWorld[1].xyz)*1000;
    float3 at = matWorld[3].xyz;
    float3 up = normalize(matWorld[2].xyz);
    float3 zaxis;
    float3 xaxis;
    float3 yaxis;
    float3 w;
...
{
    VS_OUTPUT Out;
    float3 eye = MirrorWorldMatrix[3].xyz+normalize(MirrorWorldMatrix[1].xyz);
...
float4 Basic_PS( VS_OUTPUT IN ) : COLOR0
{
	...
	return float4(IN.Color.r,0,0,1);
}
