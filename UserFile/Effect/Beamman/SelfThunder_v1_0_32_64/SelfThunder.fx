...
//バラけ具合
float particleSpread = 0.25;

//スピード
float particleSpeed = 1;

//最終到達距離
float particleSystemHeight = 0.25;

//パーティクル大きさ最大値
float particleSizeMax = 5;

//パーティクル大きさ最小値
float particleSizeMin = 0.1;

//個別回転速度
float RotateSpd = 0.25;

//全体回転速度
float GlobalRotateSpd = 0.2;
...
static float fNum = PARTICLE_COUNT * MaterialDiffuse.a;
...
{
    float2 tpos = float2(rindex % RNDTEX_WIDTH, rindex / RNDTEX_WIDTH);
    tpos += float2(0.5,0.5);
...
VS_OUTPUT FireParticleSystem_Vertex_Shader_main(float4 Pos: POSITION){
   ...
   float rad = getRandom(idx*31).x*2*3.14159265;
   float len = getRandom(idx*63).x;
   float rnd = getRandom(idx*123).x;
   float radx = sin(getRandom(idx).x*162)*2*3.14159265;
   float rady = cos(getRandom(idx).x*257)*2*3.14159265+time_0_X*GlobalRotateSpd;
   float radz = cos(getRandom(idx).x*510)*2*3.14159265;
   ...
   pos.y += t*MoveSpd*pow(Air,t);
   pos += (getRandom(idx)-0.5).rgb*particleSpread;
   ...
   view_trans_matrix = mul(view_trans_matrix, matRot);
   ...
   Pos.w = 1;
   pos += (particleSizeMax-particleSizeMin) * (pow(rnd,5)+particleSizeMin) * 1 * (Base.x * view_trans_matrix[0] + Base.y * view_trans_matrix[1]);
   pos *= scaling / 2;
   ...
   Out.texCoord = Pos.xy;
   Out.color = saturate(t);
   
   //Out.color = t*len;
   
   Out.id = idx%4;
   if ( idx >= fNum ) Out.Pos.z=-2;
   return Out;
}
...
float4 FireParticleSystem_Pixel_Shader_main(float2 texCoord: TEXCOORD0, float color: TEXCOORD1,float id: TEXCOORD2) : COLOR {
   ...
   texCoord = (texCoord*10)*0.5+0.5;
   texCoord /= 4;
   
   //現在のアニメパターン計算
   float Now = saturate(color) * AnmNum;
   float2 NowPos;
   NowPos.x = floor(Now%4);
   NowPos.y = floor(Now/4);
   
   texCoord.x += 0.25*NowPos.x;
   texCoord.y += 0.25*NowPos.y;
   
   float4 col = tex2D(Particle,texCoord);
   float w = col.r;
   col.rgb *= Col;
   col.rgb += world_matrix[3].xyz*w*0.1;
   //col.a *= color;
   //col.a = 1;
...
float4 ParticleBase_Pixel_Shader_main(float2 texCoord: TEXCOORD0) : COLOR {
   ...
   if ( idx >= fNum ) idx -= fNum;
   
   float t = frac(float(idx)/fNum + particleSpeed * time_0_X);
   ...
   if ( old_color.a <= t ) {
      ...
      return old_color;
		...
		float rnd = (getRandomPS(idx).x+time_0_X)*65535;
