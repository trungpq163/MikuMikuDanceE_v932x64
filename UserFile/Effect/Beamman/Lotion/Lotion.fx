...
int LotionDiffusePower = 32;
//UV値スケール
float LotionUVScale = 1;

float3 LotionSpecularColor = float3(0.25,0.25,0.25);
float3 LotionColor = float3(0.95,0.95,0.95); 
...
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR0
{
    ...
    //ローションの着色
    Color.rgb *= 1-((1-LotionColor)*LotionDiffPow);
    if ( useToon ) {
        // トゥーン適用
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    //ローションの着色
    ShadowColor.rgb *= 1-((1-LotionColor)*LotionDiffPow);
    Color.rgb *= 1-((1-LotionColor)*LotionDiffPow);
    // スペキュラ適用
    Color.rgb += Specular;
