...
VS_OUTPUT ParticleFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
	...
	matRot[2] = float4(sin(rady),0,cos(rady),0); 
	matRot[3] = float4(0,0,0,1); 
	float3 Add = float3(100,0,0)*index_rnd.x; 
	Add = mul(Add,matRot);
	Pos.xyz += Add;
...
VS_OUTPUT WaveFunc(float4 Pos,float3 Normal,float2 Tex)
{
	...
	matRot[2] = float4(sin(rady),0,cos(rady),0); 
	matRot[3] = float4(0,0,0,1); 
	Pos.xyz += float3(100,0,0)*index_rnd.x;
	Pos.xyz = mul(Pos.xyz,matRot);
...
VS_OUTPUT LineFunc(float4 Pos,float3 Normal,float2 Tex)
{
	...
	CenterPos.y += PosBuf.z * 3000;
	NextCenterPos *= NextCenter.x * scale;
	NextCenterPos.y += (PosBuf.z+0.0001) * 3000;
	...
	CenterPos.xyz += float3(100,0,0)*index_rnd.x;
	NextCenterPos.xyz += float3(100,0,0)*index_rnd.x;
    ...
    Out.Alpha = inv_pow(1-saturate(CenterPos.y*0.05),32);
	...
	float3 side = normalize(cross(E,V))*1.5*10*length(WorldMatrix[1])*0.1*0.5;
...
float4 Main_PS(VS_OUTPUT IN,uniform int mode) : COLOR0
{
	...
	{
		IN.Tex.xy = IN.Tex.yx;
		float4 ColA = tex2D(SampLine,(IN.Tex*float2(1,0.25))+float2(0,0.25*2));
		ColA.a *= Col.r;
		ColA.a *= 0.25;
		IN.AddTex.y = 0.25*1;
		...
		Col = ColA+ColB;
		Col = tex2D(SampLine,(IN.Tex*float2(1,0.25))+float2(0,0.25*2));
