...
{
    VS_OUTPUT Out = (VS_OUTPUT)0;
	float t0 = smoothstep(0.22,0.25,1-Tr);
...
VS_OUTPUT WaveFunc(float4 Pos,float3 Normal,float2 Tex)
{
	...
	t = 1-pow(1-t,32);
	float OutSize = t*2;//((PosBuf.x*0.5+0.5)*0.5);
...
{
    VS_OUTPUT Out = (VS_OUTPUT)0;
	float3 index_rnd = getRandom(index*123.456+123);
	//float t = smoothstep(0.1+index_rnd.y*0.1,0.2+index_rnd.y*0.1,1-Tr);
	float t = smoothstep(0.2,1.0,1-Tr);
	Out.t = t;
	...
	float3 rnd_pos = getRandom(nt+index*123.4567+PosBuf.z*10000);
	float3 rnd_pos_n = getRandom(nt+index*123.4567+PosBuf.z*10000+0.0001);
	
	float3 rnd = getRandom(nt+index*123.456+123.456+PosBuf.z*10000);
	...
	Center.y = 1.0 * (PosBuf.z != 0);//abs(Center.y)*2;
	NextCenter.y = 1.0 * (PosBuf.z != 0);//abs(NextCenter.y)*2;
	...
	*/
	...
	Out.Alpha *= saturate(pow(t,4) > 1-saturate(CenterPos.y*10));
	Out.Alpha *= 1-smoothstep(0.35,0.5+index_rnd,1-Tr);
    Out.Alpha *= 1-smoothstep(0.0,0.09,t)*0.9;
