...
float3 AddRndPos(int i)
{
	float3 ret;//(getRandom(g_index*512).xyz*2-1)*5;
	
	//ret.y *= 0.2;
	
	float fi = (float)g_index/(float)CloneNum;
	
	float3 rnd = getRandom(g_index*512).xyz;
	
	ret.x = cos(rnd.x*123.456)*0.5;
	ret.z = g_index*0.5;
	ret.y = cos(rnd.y*123.456)*0.5;
...
{
	float t = smoothstep(0+g_index*0.02,0.9+g_index*0.02,1-Tr);
...
VS_OUTPUT FireFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
    VS_OUTPUT Out = (VS_OUTPUT)0;
    Pos.xyz = Pos.xzy;
	float t = smoothstep(0+g_index*0.02,0.6+g_index*0.02,1-Tr);
	Out.t = t;
    Out.Alpha = 1.0;
	...
	Out.Pos.xyz += AddRndPos(g_index)*100;
	Out.Pos.xyz *= 0.1;
	Pos = Out.Pos;
...
VS_OUTPUT BomFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
	...
	float t = smoothstep(0+g_index*0.02,0.4+g_index*0.02,1-Tr);
...
VS_OUTPUT WaveFunc(float4 Pos,float3 Normal,float2 Tex)
{
	...
	float rad = cos(rnd.x*100)*2*3.1415;
	matRot[0] = float4(cos(rad),sin(rad),0,0); 
	matRot[1] = float4(-sin(rad),cos(rad),0,0); 
	matRot[2] = float4(0,0,1,0);
	matRot[3] = float4(0,0,0,1);
...
float4 Main_PS(VS_OUTPUT IN,uniform int mode) : COLOR0
{
	...
	{
		Col.rgb = MainColor;
		float d = tex2D(SampNoize,IN.Tex+float2(1-pow(1-IN.t,2)*0.5,1-pow(1-IN.t,2)*0.5)).r;
		d += tex2D(SampNoize,IN.Tex+float2(0.5+1-pow(1-IN.t,2)*-0.25,1-pow(1-IN.t,2)*0.5)*-0.25).r;
		
		IN.Alpha -= pow(d,4)*pow(IN.t,1.1);
	...
	if(mode == 5)
	{
		...
		float d = tex2D(SampNoize,IN.Tex+float2(1-pow(1-IN.t,2)*0.5,0)).r;
		d += tex2D(SampNoize,IN.Tex+float2(0.5+1-pow(1-IN.t,2)*-0.25,0)).r;
		IN.Alpha -= d*smoothstep(0.25,1,IN.t);

		Col.a *= tex2D(SampNoize,IN.Tex*0.5).r;
		
		IN.Alpha -= pow(d,4)*pow(IN.t,1.25);

		Col.a *= saturate(1-IN.t*1);
	...
	Col.a *= IN.Alpha;
	Col.a = saturate(lerp(0,Col.a,IN.t*10));
...
technique MainTec2 < string MMDPass = "object"; string Subset = "0";
    string Script = 
        "RenderColorTarget0=;"
	    "RenderDepthStencilTarget=;"
		"LoopByCount=CloneNum;"
