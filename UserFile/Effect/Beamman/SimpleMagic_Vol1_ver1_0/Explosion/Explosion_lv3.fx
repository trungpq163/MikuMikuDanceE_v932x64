...
VS_OUTPUT ParticleFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
	...
	add.y = abs(rnd.y)*16*(1-pow(1-t,4));
...
VS_OUTPUT FireFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
	...
	//ワールドの進行ベクトル
	float3 vec = float3(0,1,0);//normalize(r_rnd.xyz-float3(0.5,0,0.5));
	...
	vec = normalize(vec + addrotpos*1);

	Out.Pos.xyz += addrotpos*5;
	
	//カメラの位置
	float3 eyepos = view_trans_matrix[3].xyz;

	//カメラからのベクトル
	float3 eyevec = view_trans_matrix[2].xyz;
...
VS_OUTPUT SmokeFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
	...
	add.x = cos(rnd.x*2*3.1415)*(10+rnd.z*0);
	add.z = sin(rnd.x*2*3.1415)*(10+rnd.z*0);
	add.y = 0;
	Pos.xyz *= 5;
...
{
	float addt = (float)index / (float)SmokeNum2;
	addt *= 0.5;
	...
	add.x = cos(rnd.x*2*3.1415);
	add.z = sin(rnd.x*2*3.1415);
	add.y = 0;
	Pos.xyz *= 3;
...
float4 Main_PS(VS_OUTPUT IN,uniform int mode) : COLOR0
{
	...
	if(mode == 3)
	{
		...
		Col.a *= (pow(1-t,4));
		Col.rgb *= 0.25;
		Col.rgb += MainColor*pow(Col.a,4)*10;
	}
	if(mode == 7)
	{
		Col = tex2D(SampSmoke,(IN.Tex*0.25)+IN.AddTex);
		Col.rgb *= 0.25;
		Col.a *= IN.Alpha;
		Col.rgb += MainColor*pow(Col.a,1)*5;
		IN.Alpha = 1;
	}
	if(mode == 4)
	{
		Col.rgb = MainColor;
		float d = tex2D(SampNoize,IN.Tex+float2(1-pow(1-t,2)*1,0)).r;
		d += tex2D(SampNoize,IN.Tex+float2(0.5+1-pow(1-t,2)*-0.5,0)).r;
		
		IN.Alpha -= pow(d,4)*pow(t,8);
