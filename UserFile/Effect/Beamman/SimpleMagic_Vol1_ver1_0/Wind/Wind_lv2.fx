//メイン色設定
float3 MainColor = float3(0.2,1,0.5);
...
VS_OUTPUT ParticleFunc(int index,float4 Pos,float3 Normal,float2 Tex)
{
	...
	addt *= 0.3;
	float t = smoothstep(0+addt*0,0.2+addt,1-Tr);
	...
	Pos.xyz += add*lerp(len,0,saturate(1-t+r_rnd.z));//(pow(t,r_rnd.z*2)));
    Pos.xyz *= 1;
...
VS_OUTPUT WaveFunc(float4 Pos,float3 Normal,float2 Tex)
{
    ...
    addt /= WaveNum;
    addt *= 0.3;
	float t = smoothstep(0.025+addt,0.4+addt,1-Tr);
	
	float3 PosBuf = Pos;
	
	float in_t = 1-pow(1-t,16);
	t = 1-pow(1-t,32);
	float OutSize = t*20*Pos.z;//((PosBuf.x*0.5+0.5)*0.5);
	float InSize = lerp(0,OutSize,in_t)*Pos.z;
