...
//コントローラの値読み込み
bool use_Cont : CONTROLOBJECT < string name = "(self)";>;
float3 ContPos : CONTROLOBJECT < string name = "(self)";string item = "センター";>;
...
float morph_len : CONTROLOBJECT < string name = "(self)"; string item = "光長さ"; >;
float morph_size : CONTROLOBJECT < string name = "(self)"; string item = "光大きさ"; >;
float morph_boke : CONTROLOBJECT < string name = "(self)"; string item = "光ぼけ"; >;
...
float4  PS_RadialBlur(float2 Tex: TEXCOORD0 ,float2 Center: TEXCOORD1,float Alpha: TEXCOORD2) : COLOR0
{
	...
	Tex.y += 0.125;
	//Color *= (1-(pow(length(Tex - Center)*((1-morph_size)*25.0),1)-1))*morph_size;
	//Color = saturate(Color);
	Color.a = (1-Color.r)*morph_size;
	Color.rgb = float3(morph_r,morph_g,morph_b);
	Color = saturate(Color);
	if(Alpha < 0)
		Color = 0;
	return Color;
...
static float2 SampStep_G = (float2(morph_boke*5,morph_boke*5)/ViewportSize);
