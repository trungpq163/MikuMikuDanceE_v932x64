...
//パーティクルの広がり（全体の大きさ）
float particleSpread = float( 10.0 );
//パーティクル速度
float particleSpeed = float( 1.0 );
//パーティクルサイズ
float particleSize = float( 10 );
//全体の回転速度
float RotateSpd = 2;
...
VS_OUTPUT FireParticleSystem_Vertex_Shader_main(float4 Pos: POSITION){
   ...
   float radx = sin(getRandom(idx)*162)*2*3.14159265*RotateSpd;
   float rady = cos(getRandom(idx)*257)*2*3.14159265+time_0_X*0.5*RotateSpd;
   float radz = cos(getRandom(idx)*510)*2*3.14159265*RotateSpd;
   ...
   pos += base_pos.xyz*(t)+world_matrix[3].xyz*(1-t);
