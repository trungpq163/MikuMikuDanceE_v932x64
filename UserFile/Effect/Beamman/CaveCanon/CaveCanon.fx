...
float3 calcParticle(int idx,float3 Pos,float t)
{
   ...
   float radx = getRandom(idx*31)*3.1415*2*1;
   float radz = getRandom(idx*63)*3.1415*2*1;
   */
   float rady = 90*3.1415/180;
   float radx = cos(fid*3.1415*2)*3.1415*4*AUTO_ROTATE;
   float radz = 0;
   radx += (idx%4)*90*3.1415/180.0;
...
VS_OUTPUT FireParticleSystem_Vertex_Shader_main(float4 Pos: POSITION){
   ...
   float a = t+pow(t,2);
   t = lerp(0,t,(1-(Pos.y*ParticleLen*0.5+0.5)));
   //パーティクルの現在座標を計算
   ...
   Out.id = idx%4;
   Out.PatCol = ParticleColor[idx%4];
...
float4 FireParticleSystem_Pixel_Shader_main(float2 texCoord: TEXCOORD0, float color: TEXCOORD1,float id: TEXCOORD2,float3 PatCol: TEXCOORD3) : COLOR {
