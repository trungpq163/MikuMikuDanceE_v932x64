...
//エッジのぼかし
static float Gause = p_Gause * m_Gause;
...
texture EdgeDepthRT: OFFSCREENRENDERTARGET <
    string Description = "OffScreen RenderTarget for MultiEdge_DrawZ";
...
texture EdgeNormalRT: OFFSCREENRENDERTARGET <
    string Description = "OffScreen RenderTarget for MultiEdge_DrawNormal";
...
texture EdgeMask: OFFSCREENRENDERTARGET <
    string Description = "OffScreen RenderTarget for MultiEdge_EdgeMask";
    float4 ClearColor = { 0, 0, 0, 0 };
    float2 ViewPortRatio = {1.0,1.0};
    string Format = "R32F" ;
    float ClearDepth = 1.0;
...
{   
	//return tex2D(GYSamp,Tex);
	float4 col;
	float edge = tex2D(GYSamp,Tex).r;
	//return float4(edge,0,0,1);
	...
	for(int i=0;i<4;i++)
	{
		...
		Mask += tex2D(MaskSamp,Tex + test[i]*SampStep*0).r;
	}
	Mask /= 4.0;
	buf /= 4.0;
	*/
	buf += tex2D(ScnSamp,Tex);	
	Mask += tex2D(MaskSamp,Tex).r;
	...
	col = lerp(col,saturate(col + 1 * m_DispCol),Mask);
...
float4 PS_passX( float2 Tex: TEXCOORD0 ) : COLOR {   
	...
	Color  = WT_0 *   tex2D( GYSamp, Tex );
	Color += WT_1 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x  ,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x  ,0) ) );
	Color += WT_2 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x*2,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x*2,0) ) );
	Color += WT_3 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x*3,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x*3,0) ) );
	Color += WT_4 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x*4,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x*4,0) ) );
	Color += WT_5 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x*5,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x*5,0) ) );
	Color += WT_6 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x*6,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x*6,0) ) );
	Color += WT_7 * ( tex2D( GYSamp, Tex+float2(SampStep_G.x*7,0) ) + tex2D( GYSamp, Tex-float2(SampStep_G.x*7,0) ) );
...
static float2 SampStep_Fat = (float2(1,1)/ViewportSize);

float4 PS_EdgeFat(float2 Tex: TEXCOORD0) : COLOR
{   
	SampStep_Fat *= LineSize;
    float4 Color;
...
> {
    ...
    pass Last < string Script= "Draw=Buffer;"; > {
        AlphaBlendEnable = TRUE;
        SRCBLEND = SRCALPHA;
        DESTBLEND = INVSRCALPHA;
        VertexShader = compile vs_3_0 VS_Base();
