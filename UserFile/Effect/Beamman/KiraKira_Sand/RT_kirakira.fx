...
float4 getRandom(float rindex)
{
    ...
    float2 tpos2 = float2(rindex % RNDTEX_WIDTH, trunc(rindex / RNDTEX_WIDTH));
    tpos2 += float2(0.5,0.5);
    tpos2 /= float2(RNDTEX_WIDTH, RNDTEX_HEIGHT);
    
    tpos2 = cos(tpos2);
    
    float4 rndcol = tex2Dlod(rnd, float4(tpos,0,1)) + tex2Dlod(rnd, float4(tpos2,0,1));
    rndcol /= 2;
...
VS_OUTPUT Mask_VS(float4 Pos : POSITION, float2 Tex : TEXCOORD0)
{
    ...
    Pos.xyz *= ParticleSize * morph_patscale;
    ...
    //ノイズ付加
    base_pos.xz += (sin(ftime * 0.2 + index*0.01) + cos(ftime * 0.5 + index*0.01) * 0.5)  * (NoizeLevel*morph_rand);
...
float3 ControllerPos2 : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "センター"; >;
float morph_r : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "赤"; >;
float morph_g : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "緑"; >;
float morph_b : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "青"; >;
float morph_scale : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "光量"; >;
float morph_g_scale : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "範囲拡大"; >;
...
float3 PointLightColor(float4 p, float3 n)
{
	...
	float4 PL1_Cf = float4(0,1,0,1);
	float PL1_Power = 1;
	float PL1_Scaling = (1-morph_scale)*32*(1+morph_g_scale*64);
	PL1_Scaling *= (0+abs(cos(time_0_X*0.5)));
	float3 PL1_Color = float3(1-morph_r,1-morph_g,1-morph_b);
    col += 0.25*PointLightCf(length(p - ControllerPos2), PL1_Cf, PL1_Power) * PL1_Scaling * PL1_Color;
...
float4 Mask_PS( VS_OUTPUT input ) : COLOR0
{
    ...
    color.a *= input.Alpha;
    color.rgb *= PointLightColor(input.WPos,0);
    return color;
}
