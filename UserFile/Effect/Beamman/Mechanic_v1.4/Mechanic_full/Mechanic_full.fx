//メカっぽいシェーダ_full ver1.4
//ビームマンP

//フィルライト色
float3 FillLight = float3(100,75,50)/2/255.0;
//バックライト色
float3 BackLight = float3(100,92,82)/2/255.0;
//空色
float3 SkyColor = float3(0.9f, 0.9f, 1.0f)/2;
//地面色
float3 GroundColor = float3(0.1f, 0.05f, 0.0f)/2;
//明るさ補正値
float LightParam = 1;
//自己発色抑制値
...
float4 BufferShadow_Mec_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
	...
	if ( useTexture ) {
		float2 BaseTex = tex;
		float3 Eye = normalize(CameraPosition.xyz - IN.WorldPos.xyz);
		height = tex2D( ObjTexSampler, tex+float2(0.5,0.5)).r;
		tex = tex - Eye.xy * height*0.0001;
        tex = clamp(tex,0,0.4995);
...
technique MainTecBS8  < string MMDPass = "object_ss"; bool UseTexture = true; bool UseSphereMap = false; bool UseToon = false; > {
    pass DrawObject {
