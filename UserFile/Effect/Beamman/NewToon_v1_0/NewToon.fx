...
float AddShadow = 1;
//追加影強さ
float AddShadowPow = 1;
//追加ハイライト濃さ
float AddLight = 1.0;
//追加ハイライト強さ
float AddLightPow = 1.0;
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
	...
	work = pow(saturate(work*1.5),AddShadowPow);
	ShadowColor.rgb = lerp(ShadowColor.rgb,pow(ShadowColor.rgb*AddShadow,2),work);
	
	//追加ハイライト色計算
	work = saturate(dot( IN.Normal, -LightDirection ));
	work = pow(saturate(work),AddLightPow);
	Color.rgb = lerp(Color.rgb,Color.rgb * AddLight,work);
