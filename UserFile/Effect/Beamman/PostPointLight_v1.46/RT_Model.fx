...
float3 ControllerPos : CONTROLOBJECT < string name = "(OffscreenOwner)"; string item = "センター"; >;
...
float3 PointLightColor(float4 p, float3 n)
{
    ...
    col += max(0,dot( n, normalize(ControllerPos - p) )) * 0.2 * PointLightCf(length(p - ControllerPos), PL1_Cf, PL1_Power) * PL1_Scaling * PL1_Color;
...
{
	float3 LightCol = float3(1-morph_r,1-morph_g,1-morph_b);
    ...
    float3 wLightDirection = normalize(IN.LastPos - ControllerPos);
    // 点光源からの光
