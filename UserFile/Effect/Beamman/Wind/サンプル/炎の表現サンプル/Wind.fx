...
//風ひとつひとつの高さ最大値
float LocalHeight = 0.75;

//風の広がり強さ
float WindSizeSpd = 2;

//広がりの乱数幅
float WindSizeRnd =0.5;

//テクスチャ繰り返し数
float ScrollNum = 1;

//色設定
float3 Color = float3( 0.8, 0.25, 0 );
...
VS_OUTPUT lineSystem_Vertex_Shader_main(float4 Pos: POSITION){
   VS_OUTPUT Out;
 float fclone = CloneNum;
    float findex = index;
	float offset = findex*(ThunderWaitTime / (fclone*ThunderWaitTime)) + sin(findex)*ShotRandOffset;
	float time_buf = time_0_X*AnmSpd + offset;
    float t = time_buf*60;
 
 
 //ランダム値
  float3 rand = tex2Dlod(rnd, float4((findex/fclone)*11,0,0,1));
   t %= ThunderWaitTime;
...
float4 lineSystem_Pixel_Shader_main(float2 texCoord: TEXCOORD0,float color: TEXCOORD1,float alpha: TEXCOORD2,float2 CenterTex: TEXCOORD3,float2 DistTex: TEXCOORD4) : COLOR {
   ...
   float4 col = float4(tex2D(AuraTex1Sampler,texCoord+float2(color*0.001,0)));   
