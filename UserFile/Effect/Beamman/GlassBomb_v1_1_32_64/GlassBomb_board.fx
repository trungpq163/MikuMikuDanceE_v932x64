...
VS_OUTPUT BombVS(float4 Pos: POSITION,float2 Tex: TEXCOORD0,float3 Normal : NORMAL){
	...
	float3 Vec = float3(-0.5+r2.x,0,-0.5+r2.y)*100+float3(0,sca*sin(theta)*st*(1+AddWidth),sca*cos(theta)*-st*(1+AddHeight));

	float4x4 matRot;
	
    float wt = saturate(t*2*r2.y);
	//Y軸回転 
	...
	float3 w = (1+AddScale)*1*(((1+r2.y*5) + 1 * AddPatScale) * float3(Pos.xy - float2(0,0),0));
	...
	world_matrix[3].xyz = 0;
	w = mul(float4(w, 1),world_matrix);
