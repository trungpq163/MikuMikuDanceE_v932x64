...
//モーフ値を取得
float morph_eye_my : CONTROLOBJECT < string name = "(self)"; string item = "MMEうる目"; >;

//コントローラのモーフ値も取得
float morph_eye_cont : CONTROLOBJECT < string name = "うる目コントローラ_0.pmd"; string item = "MMEうる目"; >;

static float morph_eye = saturate(morph_eye_my + morph_eye_cont);

//ハイライトアニメ用パラメータ
static float2 AnmScale = float2(0.05,0.05)*(1-morph_eye);
float AnmSpd = 3;
...
//目ベース
texture2D EyeBase <
    string ResourceName = "eye_base.png";
>;
sampler EyeTexSamp = sampler_state {
    texture = <EyeBase>;
    MINFILTER = LINEAR;
    MAGFILTER = LINEAR;
};
...
texture2D Hightlight <
    string ResourceName = "highlight.png";
>;
sampler HightlightSamp = sampler_state {
    texture = <Hightlight>;
    MINFILTER = LINEAR;
    MAGFILTER = LINEAR;
};
//ハイライト移動用
texture2D RandTex <
    string ResourceName = "RandTex.png";
>;
sampler RndSamp = sampler_state {
    texture = <RandTex>;
    MINFILTER = LINEAR;
    MAGFILTER = LINEAR;
};
...
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR0
{
    ...
    if ( useTexture ) {
        // テクスチャ適用
        float2 Highlight_Anm = AnmScale*len*sin(tex2D(RndSamp,float2(time*AnmSpd,0.5)));
        float4 TexColor = tex2D( EyeTexSamp, IN.Tex )+tex2D( HightlightSamp,IN.Tex+Highlight_Anm);
        Color *= TexColor;
    }
    if ( useSphereMap ) {
        // スフィアマップ適用
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    //カメラとの距離
    float len = 1;//length(IN.Eye)*LenParam;
