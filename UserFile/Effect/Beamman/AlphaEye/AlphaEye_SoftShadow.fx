...
texture BaseRT: OFFSCREENRENDERTARGET <
    string Description = "OffScreen RenderTarget for AlphaEye.fx";
...
sampler BaseView = sampler_state {
    texture = <BaseRT>;
    Filter = LINEAR;
    AddressU  = CLAMP;
    AddressV = CLAMP;
...
texture AlphaRT: OFFSCREENRENDERTARGET <
    string Description = "OffScreen RenderTarget for AlphaEye.fx";
    float4 ClearColor = { 0, 0, 0, 1 };
    float ClearDepth = 1.0;
    bool AntiAlias = true;
...
sampler AlphaMaskView = sampler_state {
    texture = <AlphaRT>;
    Filter = LINEAR;
    AddressU  = CLAMP;
    AddressV = CLAMP;
...
{
	AlphaParam = saturate(AlphaParam+AlphaMorph);
	...
	float4 AMask = tex2D(AlphaMaskView,TgtPos.xy);
	float4 BaseCol = tex2D(BaseView,TgtPos.xy);
    ...
    Color = lerp(Color,BaseCol,pow(AMask.r,8)*(1.0 - AlphaParam));
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    if( any( saturate(TransTexCoord) != TransTexCoord ) ) {
        // シャドウバッファ外
        Color = lerp(Color,BaseCol,pow(AMask.r,8)*(1-AlphaParam));
        return Color;
    } else {
        float comp = 0;
    	...
    	ans = lerp(ans,BaseCol,pow(AMask.r,8)*(1-AlphaParam));
        return ans;
    }
}
