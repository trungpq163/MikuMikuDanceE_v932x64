//水面の分割数（細かさ制御用）
//--！重要！--ここの数値はMirrorWaterと同じものを設定します--/
int WaveSplitLevel = 32;
...
float morph_cc_h : CONTROLOBJECT < string name = "ColorController.pmd"; string item = "乗算色相"; >;
float morph_cc_s : CONTROLOBJECT < string name = "ColorController.pmd"; string item = "乗算彩度"; >;
float morph_cc_b : CONTROLOBJECT < string name = "ColorController.pmd"; string item = "乗算明度"; >;
//HSB変換用色テクスチャ
...
//フォグ色
float3 FogCol_base = float3(0.97,0.98,0.99);
static float3 FogCol = FogCol_base*float3(1-morph_r,1-morph_g,1-morph_b);
...
VS_OUTPUT Basic_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
    ...
    CameraPosition.y = MirrorWorldMatrix[3].y;
    ...
    if ( useSphereMap ) {
        ...
        //Out.SpTex.x = NormalWV.x * 0.5f + 0.5f; // PiT mod below 
        //Out.SpTex.y = NormalWV.y * -0.5f + 0.5f; // PiT mod below
	Out.SpTex.x = (NormalWV.x + 1.0f)* 0.5f; 
        Out.SpTex.y = (1.0f - NormalWV.y) * -0.5f; 
    } 
...
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR0
{
	float4 pallet;
	//乗算
	pallet = tex2D(PalletSamp,float2(morph_cc_h,1-morph_cc_s))*saturate(1-morph_cc_b*0.5)*2;
	FogCol.rgb *= pallet.rgb;
    ...
    float EyeLen = abs(length(IN.Eye));

	//火線表現
	//現在の座標、法線から対象の水面を得る
	
	float3 WN = tex2D(NormalSampler,(IN.WPos.xz+(1+IN.WPos.y)+IN.Normal*64)/length(MirrorWorldMatrix[0])*WaveSplitLevel).rgb;
	
	WN.g *= -1;
	WN = WN.rgb;
		
	//WN = 2.0f * WN - 1.0f; //PiT mod below
	WN = WN - 0.5f;
	WN = normalize(WN);
	
	WN = pow(dot(-LightDirection,WN)*1.15,1)*Caustics*0.1;
	WN = max(WN - (EyeLen*FogPow)*(1-FogCol),0);
	
	Color.xyz += WN;
	    
    Color.rgb -= (EyeLen*FogPow)*(1-FogCol);
...
BufferShadow_OUTPUT BufferShadow_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
	...
	if(use_light)
	{
		...
		LightAmbient.g = 1-morph_g;
		LightAmbient.b = 1-morph_b;	
		AmbientColor = saturate(MaterialAmbient  * LightAmbient + MaterialEmmisive);
	}
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
	...
	float3 WN = tex2D(NormalSampler,(IN.WPos.xz+(1+IN.WPos.y)+IN.Normal*1)/length(MirrorWorldMatrix[0])*WaveSplitLevel).rgb;
	...
	WN = pow(dot(-LightDirection,WN)*1.15,32)*0.1*Caustics;
    ...
    Color.rgb -= (EyeLen*FogPow)*(1-FogCol);
    ShadowColor.rgb -= (EyeLen*FogPow)*(1-FogCol);
