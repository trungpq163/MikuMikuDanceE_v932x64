...
float CutSpeed = 0;
//色
float4 ParticleColor = float4(1,1,1,0.25);
//スペキュラ色
...
texture LineDistParticle_DistRT: OFFSCREENRENDERTARGET <
    string Description = "OffScreen RenderTarget for LineDistParticle.fx";
...
sampler DistortionView = sampler_state {
    texture = <LineDistParticle_DistRT>;
    AddressU  = CLAMP;
    AddressV = CLAMP;
};
...
float4 FireParticleSystem_Pixel_Shader_main(VS_OUTPUT IN) : COLOR {
   float4 col = tex2D(Particle,(IN.texCoord*10)*0.5+0.5);
	...
	col.rgb = lerp(ret.rgb,ParticleColor.rgb,ParticleColor.a);
	
    float3 HalfVector = normalize( normalize(view_trans_matrix[2].xyz) + -LightDirection );
    float LotionPow = pow( max(0,dot( HalfVector, normalize(normal) )), 8 );
    float3 Specular = LotionPow * SpecularColor;
    col.a *= IN.color;
    col.rgb += Specular;
    return col;
