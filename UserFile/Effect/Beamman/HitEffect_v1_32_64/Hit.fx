...
VS_OUTPUT AllFunc(int num,float4 Pos,float Scale,float ParticleLen,float AlphaSpd,float rndadd,float up)
{
   ...
   //通常回転
   //回転行列の作成
   float rad = (time_0_X * RotateSpd*(0.5-r) + t + cos(62 * r + fi)) * (index != 0 ? 1 : -1);
   float4x4 matRot;
...
VS_OUTPUT ParticleFunc(int num,float4 Pos,float Scale,float ParticleLen,float AlphaSpd,float rndadd)
{
   ...
   //通常回転
   //回転行列の作成
   float rad = t * RotateSpd + cos(62 * fi)*2*3.1415;
   float4x4 matRot;
   matRot[0] = float4(cos(rad),sin(rad),0,0); 
   matRot[1] = float4(-sin(rad),cos(rad),0,0); 
   matRot[2] = float4(0,0,1,0); 
...
VS_OUTPUT FlashVS(float4 Pos: POSITION){
   ...
   {
   		Out = AllFunc(FlashNum,Pos,10,0,0.9,0,0);
	    Out.texCoord.y += 0.5;
   }
   Out.ColBoost = 8.0;
   return Out;
...
VS_OUTPUT SmokeVS(float4 Pos: POSITION){
   ...
   if(MaterialDiffuse.a != 0 && MaterialDiffuse.a != 1)
   {
   		Out = AllFunc(SmokeNum,Pos,1,0.1,0,1,50);
	    Out.texCoord.x += 0.5;
   }
   return Out;
}
VS_OUTPUT FireVS(float4 Pos: POSITION){
   ...
   if(MaterialDiffuse.a != 0 && MaterialDiffuse.a != 1)
   {
   		Out = AllFunc(FireNum,Pos,0.3,1,0,0.1,1);
	    Out.texCoord.x += 0.5;
   }
   return Out;
}
...
float4 AllPS(float2 texCoord: TEXCOORD0, float color: TEXCOORD1,float ColBoost: TEXCOORD2) : COLOR {
