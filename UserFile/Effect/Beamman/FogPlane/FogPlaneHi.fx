...
float2 FogScroll = float2(0.1,0);

float2 Wind = float2(0.01,0.01);
float FogSpd = float(0.2);
float TexScale = float(1);

float FogAddAlpha = 0.0;
float FogAlpha = 32;

float FogGausePow = 10;
float FogGauseSpd = 0.95;
float FogPushPow = 10;

float FogHeight = 0.15;
...
{
    VS_OUTPUT Out;
	Pos.y += (-0.5+tex2Dlod(FogMap,float4(Tex+FogScroll*time*FogSpd,0,0)).r)*FogHeight;
    Out.DefPos = Pos;
...
float4 Mask_PS( VS_OUTPUT IN ,uniform bool bShadow) : COLOR0
{
	...
	IN.Tex *= TexScale;
	time *= FogSpd;
	float2 tex = IN.Tex+FogScroll*time*FogSpd;
	float3 normal;
...
VS_ZValuePlot_OUTPUT ZValuePlot_VS( float4 Pos : POSITION )
{
	...
	Out.Tex = (Pos.xz*0.5+0.5);
	Pos.y += tex2Dlod(FogMap,float4(Out.Tex+FogScroll*time*FogSpd,0,0)).r*0.1;
...
{
	float hit = pow(tex2D(HitMapBuf2,IN.Tex).r,1)*FogPushPow*21;
    // R色成分にZ値を記録する
    return float4(IN.ShadowMapTex.z/IN.ShadowMapTex.w,0,0,1-hit);
}
