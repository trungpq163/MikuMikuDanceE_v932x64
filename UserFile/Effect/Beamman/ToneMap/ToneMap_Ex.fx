...
//ぼかし係数
float GauseParam = 1.0;
//トーンマップの強さ
static float ToneParam = 5*0.6*MaterialDiffuse.a;
...
texture HDRMap: OFFSCREENRENDERTARGET <
    string Description = "HDROBj_Tgt for ToneMap_Ex.fx";
    float2 ViewPortRatio = {1.0,1.0};
...
float4 PS_passLuminance( float2 Tex: TEXCOORD0 ) : COLOR {   
	...
	Color = tex2D( HDRSamp,Tex)*LuminancePow;
...
{   
    float4 Color;
    float Tone = (GetTone()*ToneParam+0.5*1);
