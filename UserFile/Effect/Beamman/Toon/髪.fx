...
{
	float d = saturate(dot(normalize(IN.Normal),-LightDirection));
    ...
    float3 BuffNormal = float3(0,0.55,-1);
    float3 BuffNormal2 = IN.Normal;
    //BuffNormal2.y = normalize(BuffNormal).y;
    BuffNormal = lerp(normalize(BuffNormal),normalize(BuffNormal2),1);
    
    float s = pow( max(0,dot( HalfVector, normalize(BuffNormal) )), HalfVector.y * HalfVector.y );
    s = pow(s,1);
    s = (s < 0.9995) ? 0 : 1;
    
    //return float4(s,s,s,1);
    
    float3 Specular = s * SpecularColor;
    ...
    if ( useSphereMap ) {
        ...
        //TexColor.r = (TexColor.r > 0.1) ? 1 : 0;
        
//        TexColor.r *= saturate(pow(IN.WPos.y*0.055,32));
//        TexColor.rgb = TexColor.r * float3(1.0, 0.9, 0.9);
    	TexColor.rgb = max(pow(TexColor.rgb,16), 0) * 10;
//        TexColor.rgb = saturate(TexColor.rgb)*0.5;
        if(spadd) {
            Color.rgb += TexColor.rgb;
            ShadowColor.rgb += TexColor.rgb * SpecularStrength;
        } else {
            Color.rgb *= TexColor.rgb;
            ShadowColor.rgb *= TexColor.rgb * SpecularStrength;
        }
    ...
    Color.rgb = lerp(Color.rgb,Color.rgb * MaterialToon * ShadowStrength * ShadowBiasColor + ShadowBiasColorAdd, d) * BaseBiasColor + BaseBiasColorAdd;
