...
float3 BaseBiasColor = float3(1.05, 1.05, 1.05);
...
BufferShadow_OUTPUT BufferShadow_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
    ...
    // カメラ視点のワールドビュー射影変換
    Out.Pos = mul( Pos, WorldViewProjMatrix );
    float3 BufNormal = Normal;
	BufNormal.z = -1;
	BufNormal.y = 0.25;
	BufNormal.x = 0;
	BufNormal = normalize(BufNormal);
	BufNormal = mul(BufNormal,HeadMat);
	Normal = lerp(Normal,BufNormal,0.5);
