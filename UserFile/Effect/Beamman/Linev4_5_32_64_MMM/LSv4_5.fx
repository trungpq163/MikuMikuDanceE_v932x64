...
//生成速度なめらかさ（↑のCutSpeedが0の時は何もおきないです）
float CutPow
<
   string UIName = "CutPow";
   string UIWidget = "Numeric";
   bool UIVisible =  true;
   float UIMin = 0;
   float UIMax = 32;
...
static float LINE_LENGTH = Length;

#define LINE_CROSS

#define ADD_UV (0.5/TEX_WIDTH)
...
VS_OUTPUT lineSystem_Vertex_Shader_main(float4 Pos: POSITION,uniform float rad){
   ...
   //現在の座標を取得
   float2 base_tex_coord = float2(idx%TEX_WIDTH,trunc(idx / TEX_WIDTH));
   base_tex_coord += float2(0.5,0.5);
   base_tex_coord /= float2(TEX_WIDTH, TEX_HEIGHT);
   ...
   if(BILLBORAD)
   {
	   ...
	   for(int i = 1;i<LINE_LENGTH;i++)
	   {
		   ...
		   int tgt = idx+i;
		   //次ＩＤのマトリクスを取得
		   float2 tgt_tex_coord = float2(tgt%TEX_WIDTH,trunc(tgt / TEX_WIDTH));
		   tgt_tex_coord += float2(0.5,0.5);
		   tgt_tex_coord /= float2(TEX_WIDTH, TEX_HEIGHT);
...
float4 WorldBase1_Pixel_Shader_main(float2 texCoord: TEXCOORD0) : COLOR {
   ...
   int idx = round(texWork.x*TEX_WIDTH)+TEX_WIDTH * round(texWork.y*TEX_HEIGHT);
...
float4 AlphaBase_Pixel_Shader_main(float2 texCoord: TEXCOORD0) : COLOR {
   ...
   base_tex_coord /= float2(TEX_WIDTH, TEX_HEIGHT);
   idx -= 1;
   float2 base_tex_coord_prev = float2(idx%TEX_WIDTH,trunc(idx / TEX_WIDTH));
   base_tex_coord_prev += float2(0.5,0.5);
   base_tex_coord_prev /= float2(TEX_WIDTH, TEX_HEIGHT);
   ...
   if(idx == 0 || time_0_X < 0.05)
   {
   		...
   		float3 PrevPos = tex2D(WorldBase4_buf, base_tex_coord_prev).xyz;
