#define CONTROLLER_NAME "material_vinyl.pmx"
#include "material_vinyl_editor.fxsub"


static const float transparent_factor = mTransFactor + 0.1;	////透明度の係数　0～1
static const float transparent_level = mTransLevel + 0.3;  //透明度をかさ上げ　0～1（通常0）
static const float transparent_reverse = mTransReverse + 0.1; //法線に従って色が濃くなる範囲の指定　-1～1(通常0)　メッシュ形状に合わせて都合が良い値にする

static const float specular_boost_factor = mSpecBoost + 0.6; //スペキュラをかさ上げする係数　0～1　見た目の透明度は下がる
static const float reflection_boost_factor = mRefBoost + 0.7; //反射をかさ上げする係数　0～1 見た目の透明度は下がる
static const float revese_surface_factor = mRevFactor + 0.1;
