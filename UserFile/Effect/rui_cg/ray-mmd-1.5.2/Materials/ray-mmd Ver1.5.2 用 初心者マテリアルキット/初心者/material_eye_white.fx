#define CONTROLLER_NAME "material_eye_white.pmx"
#include "../Editor/Glass/material_editor.fxsub"
...
static const float3 albedo = mAlbedoColor * 1.05;
...
static const float3 albedoSub = mMelanin * 0.5;
...
#define NORMAL_MAP_FILE "NormalMap/NMeye.png"

static const float normalMapScale = -0.1* (mNormalScale - 1) - 2.0;
static const float normalMapLoopNum = 4 * (mNormalLoops - 1) + 1.0;
...
static const float normalSubMapScale = -1 * (mNormalSubScale - 1) - 0.0;
...
static const float smoothness = 4 * (mSmoothness - 1) + 2.0;
...
static const float metalness = (mMetalness - 1) + 0.01;
...
static const float3 specular = (mSpecularColor - 1) + 0.04;
...
static const float3 emissive = float3(100, 100, 100);
static const float emissiveBlink = mEmissiveBlink; 
static const float emissiveIntensity = 0.2 * (mEmissiveIntensity - 1) + 0.2;
