/*--------------------------------------------
【カスタマイズ用黒肌マテリアルfx】
このマテリアルfxはカスタマイズ用に作成したものです。
自分好みの色を作りたい場合にご利用ください。
編集する場合はMMDでモデルにこのマテリアルfxを適用した状態で行うと、
編集結果が直ぐに反映されます。

【解説】
編集箇所のカッコ内の数値、"(255, 255, 255)"の部分を変更し、
保存することで肌色を変えることができます。

左から順に、赤、緑、青を表し、
各色に0から255までの数値を入れることが出来ます。
255が最も鮮やかで0が発色無しになります。
数値を全て0にすると(0, 0, 0)、黒になります。
...
#define SSS_SKIN_TRANSMITTANCE(x) exp((1 - saturate(x)) * float3(-47, -47, -47))
