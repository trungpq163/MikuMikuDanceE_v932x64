////////////////////////////////////////////////////////////////////////////////////////////////
//
//  NumbersPmd_Float4x4.fx ver0.0.4 数値データの表示(float4x4型)
//  作成: 針金P
//
////////////////////////////////////////////////////////////////////////////////////////////////
// ここのパラメータを変更してください
float4x4 WldViwPrjMat : WORLDVIEWPROJECTION;

// ここに表示させる数値､または変数,式を代入(正確に読めるのは全7桁程度)
static float4x4 Value = WldViwPrjMat;

int FractCount = 3;     // 少数以下桁数
float XDistance = 0.9;  // 数字の横方向間隔
...
static int iValue = round(Value[ObjIndexY][ObjIndexX] * pow(10.0f, FractCount));
...
VS_OUTPUT Obj_VS(float4 Pos : POSITION, float2 Tex : TEXCOORD0, int index: _INDEX)
{
    ...
    // ボード配置
    Pos.x *= 0.5f;
    Pos.x += 10.0f * (ObjIndexX - 1.0f) * XDistance - float(Index);
    Pos.y -= ObjIndexY * 2.0f - 3.0f;
...
technique MainTec0 < string MMDPass = "object";
    string Script = "LoopByCount=CountY;"
                       "LoopGetIndex=ObjIndexY;"
                       "LoopByCount=CountX;"
                          "LoopGetIndex=ObjIndexX;"
                          "Pass=DrawObject;"
                       "LoopEnd=;"
                    "LoopEnd=;"; >
{
