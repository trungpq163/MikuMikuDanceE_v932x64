ActiveParticleSmoke.fx ver0.0.8
...
・基本的な使用方法
(1)ActiveParticleSmoke.xをMMDにロードしてください．
(2)描画順序はできるだけ後の方にしてください．
(3)ActiveParticleSmoke.xを適当なボーンに付けて動かすと,動いた軌道上に煙の尾が描画されます．
...
・追加粒子描画について
ActiveParticleSmoke.fxの先頭パラメータ UNIT_COUNT0 を1以上にするとオブジェクト粒子発生口にメインの煙描画とは別に
別系統の煙を描画することが出来ます。粒子発生直後の途切れをカバーしたり、ミサイル噴射口の閃光等に利用出来ます。


・シャドウマップ描画について
ActiveParticleSmoke.xのアクセサリパラメータの｢影｣にチェックを入れると煙のシャドウマップ描画を行い、
背景モデルに煙の影を描画することが出来ます。煙自体に影は描画されません。また、いわゆる煙のモヤッとした
影にはならないのであまり期待はしないように。
