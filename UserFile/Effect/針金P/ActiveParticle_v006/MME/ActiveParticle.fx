////////////////////////////////////////////////////////////////////////////////////////////////
//
//  ActiveParticle.fx ver0.0.6 オブジェクトの移動に応じてテクスチャ粒子を放出します
//  作成: 針金P
//
////////////////////////////////////////////////////////////////////////////////////////////////
// ここのパラメータを変更してください
...
// テクスチャ設定
#define TEX_FileName  "sample1.png" // 粒子に貼り付けるテクスチャファイル名
...
float ParticleSize = 0.7;           // 粒子大きさ
float ParticleRandamSize = 0.0;     // 粒子サイズばらつき度(0.0～1.0)
float ParticleSpeedMin = 1.0;       // 粒子初期最小スピード
float ParticleSpeedMax = 2.5;       // 粒子初期最大スピード
float ParticleRotSpeedMin = 0.0;    // 粒子回転速度範囲最小値
float ParticleRotSpeedMax = 0.0;    // 粒子回転速度範囲最大値
float ParticleRotRandam = 0.0;      // 粒子発生時回転ばらつき度(0.0～1.0)
...
VS_OUTPUT2 Particle_VS(float4 Pos : POSITION, float2 Tex : TEXCOORD0)
{
   ...
   float rand2 = 0.5f * (0.38f * sin(55.1f * Index0) + 0.62f * cos(44.4f * Index0) + 1.0f);
   float rand3 = 0.5f * (0.43f * sin(77.1f * Index0) + 0.57f * cos(33.3f * Index0) + 1.0f);

   // 粒子の大きさ
   Pos.xy *= (1.0f + ParticleRandamSize * (2.0f * rand0 - 1.0f)) * ParticleSize * 10.0f;

   // 粒子の回転
   float rot = etime * lerp(ParticleRotSpeedMin, ParticleRotSpeedMax, rand1) + 6.18f * ParticleRotRandam * (rand2-0.5f);
