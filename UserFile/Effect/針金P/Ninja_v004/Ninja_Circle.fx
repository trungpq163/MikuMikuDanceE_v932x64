////////////////////////////////////////////////////////////////////////////////////////////////
//
//  Ninja_Circle.fx ver0.0.4   分身の術(円形配置)
//  作成: 針金P( 舞力介入P氏のfull.fx,そぼろ氏のClone.fx改変 )
//
////////////////////////////////////////////////////////////////////////////////////////////////
// ここのパラメータを変更してください

int Num = 7;              // 分身する数
int Fps = 10;             // 瞬間移動する時間間隔(fpsで入力)
int Count = 3;            // 同時に表示される数
int Step = 2;             // 同時に表示されるステップ数
float2 Origin = {0, 0};   // 配置円中心座標{x,z}
float Radius = 35;        // 配置円半径
float AngleInterval = 52; // 配置間隔角度(degree)
...
////////////////////////////////////////////////////////////////////////////////////////////////
// 配置移動座標変換行列(Indexが複製番号)
float4x4 GetTransMatrix()
{
   float ds = radians(AngleInterval);
   float s = ds * ( mod(time*Fps+Index*Step, Num) - div(Num,2) );
   float sinth = sin(s);
   float costh = cos(s);

   return  float4x4( costh,  0,                 -sinth,  0,
                         0,  1,                      0,  0,
                     sinth,  0,                  costh,  0,
     Radius*sinth+Origin.x,  0,  Radius*costh+Origin.y,  1 );
}

////////////////////////////////////////////////////////////////////////////////////////////////
// ループスクリプト

#define LOOPSCRIPT_OBJECT  "LoopByCount=Count; LoopGetIndex=Index; Pass=DrawObject; LoopEnd=;"
#define LOOPSCRIPT_EDGE    "LoopByCount=Count; LoopGetIndex=Index; Pass=DrawEdge; LoopEnd=;"
#define LOOPSCRIPT_SHADOW  "LoopByCount=Count; LoopGetIndex=Index; Pass=DrawShadow; LoopEnd=;"
#define LOOPSCRIPT_ZPLOT   "LoopByCount=Count; LoopGetIndex=Index; Pass=ZValuePlot; LoopEnd=;"
...
VS_OUTPUT Basic_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, float2 Tex2 : TEXCOORD1, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
    ...
    // 配置座標変換
    float4x4 TransMatrix = GetTransMatrix();
