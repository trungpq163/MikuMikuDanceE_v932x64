...
ActiveParticleLight_MMM.fxm ver0.0.6

オブジェクトの移動に応じてキラキラ粒子を放出します。
オブジェクトの移動量に応じて放出される粒子の数と放出位置が調整されるので,オブジェクトを
どのように動かしてもまんべんなく粒子が広がります。
ActiveParticleを元にキラキラ粒子の描画に特化させました。


・使用方法
(1)ActiveParticleLight_MMM.fxmをMMMにロードしてください。
(2)描画順序はできるだけ後の方にしてください。
(3)ActiveParticleLight_MMM.xを動かすと,動きに合わせて粒子が描画されます。
...
(2)SmoothControl.xの描画順を必ずActiveParticleLight_MMM.fxmよりも前になるようにして下さい。
...
(2)BackgroundControl.xの描画順を必ずActiveParticleLight_MMM.fxmよりも前になるようにして下さい。
...
(2)TimeControl.xの描画順を必ずActiveParticleLight_MMM.fxmよりも前になるようにして下さい。
