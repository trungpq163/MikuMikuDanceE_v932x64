...
float AcsZ  : CONTROLOBJECT < string name = "Hologram.x"; string item = "Z"; >;
static float ObjAlpha = saturate(AcsZ);
...
{
    // 輪郭色で塗りつぶし
    return float4(EdgeColor.rgb, EdgeColor.a * ObjAlpha);
}
...
{
    // 地面影色で塗りつぶし
    return float4(GroundShadowColor.rgb, GroundShadowColor.a * ObjAlpha);
}
