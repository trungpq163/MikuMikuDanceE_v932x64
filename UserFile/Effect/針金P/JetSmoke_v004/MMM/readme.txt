...
(2)SmoothControl.xの描画順を必ずJetSmoke_MMM.fxmよりも前になるようにして下さい。
(3)このアクセの裏側域では煙は非表示になります。また接地面で表示がスムージングされ切れ目が目立たなくなります。
(4)SmoothControl.xのアクセサリパラメータで接地面の位置・角度を調整出来ます、｢拡大｣でスムージング幅を調整できます。
...
(2)TimeControl.xの描画順を必ずJetSmoke_MMM.fxmよりも前になるようにして下さい。
