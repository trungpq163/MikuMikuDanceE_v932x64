...
//  full.fx ver2.0
//  作成: 舞力介入P
//  VolumeShadowで使用出来るように改変(シャドウボリューム追加モデル用) by 針金P
//
////////////////////////////////////////////////////////////////////////////////////////////////
// パラメータ宣言

////////////////// VolumeShadowでこれを追加します /////////////////////
// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓

// VolumeShadowの必要なシャドウマップパラメータを取り込む
#define SHADOWVOLUMEMODEL
#include "VolumeShadow_Header.fxh"
...
{
    ////////////////// VolumeShadowではこれを追加します ///////////////////
    // ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
    // VolumeShadow描画時はシャドウマップ描画を行わない
    clip( 0.99f - float(VolumeShadow_Valid) );
    // ↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑↑
    ///////////////////////////////////////////////////////////////////////
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    if( VolumeShadow_Valid ){
        ...
        if( !any( saturate(TransTexCoord) - TransTexCoord ) ) {
            float dist = max( min(IN.ZCalcTex.z, 1.0f) - tex2D(DefSampler,TransTexCoord).r, 0.0f );
            if(parthf) {
                // セルフシャドウ mode2
                comp=1-saturate(dist * SKII2 * TransTexCoord.y - 0.3f);
            } else {
                // セルフシャドウ mode1
                comp=1-saturate(dist * SKII1 - 0.3f);
            }
        }

        // シャドウボリュームによる遮蔽判定
        comp = min( VolumeShadow_GetSelfShadowRate(IN.PPos), comp);
        ...
        // 影色に濃度を加味する
        VolumeShadow_COLOR Out = VolumeShadow_GetShadowDensity(Color, ShadowColor, useToon, LightNormal);
