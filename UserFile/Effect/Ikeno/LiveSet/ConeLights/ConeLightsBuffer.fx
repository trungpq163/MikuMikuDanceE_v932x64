...
#define USE_16BIT_COLOR		1	// バッファの深度サイズ
#define BufferSize	0.5			// バッファの大きさ
...
texture BufferMap : RENDERCOLORTARGET
<
	float2 ViewportRatio = {BufferSize, BufferSize};
#if USE_16BIT_COLOR > 0
	string Format="A16B16G16R16F";
#else
	string Format = "A8R8G8B8";
#endif
>;
sampler BufferSmp = sampler_state
{
	Texture = <BufferMap>;
...
texture BufferDepth : RenderDepthStencilTarget <
	float2 ViewportRatio = {BufferSize, BufferSize};
	string Format = "D24S8";
>;
...
VS_OUTPUT Object_VS1( float4 Pos : POSITION, float4 Tex : TEXCOORD0)
{
	VS_OUTPUT Out = (VS_OUTPUT)0; 
	Out.Pos = Pos;
	Out.Tex = Tex.xy + ViewportOffset.xy;
	return Out;
}

float4 Object_PS1(VS_OUTPUT IN, uniform bool useTexture) : COLOR
{
	float4 Color = tex2D(BufferSmp, IN.Tex.xy);
...
#define OBJECT_TEC(name, mmdpass, tex, _subset, _mat, _br) \
	technique name < string MMDPass = mmdpass; string Subset = _subset; \
		string Script = \
			...
			"RenderColorTarget0=BufferMap; RenderDepthStencilTarget=BufferDepth;" \
		...
		pass DrawObject1 <string Script= "Draw=Buffer;";> { \
			ZENABLE = FALSE; ZWRITEENABLE = FALSE; \
			SRCBLEND = SRCALPHA; DESTBLEND = ONE; \
			VertexShader = compile vs_3_0 Object_VS1(); \
			PixelShader  = compile ps_3_0 Object_PS1(tex); \
		} \
	}
