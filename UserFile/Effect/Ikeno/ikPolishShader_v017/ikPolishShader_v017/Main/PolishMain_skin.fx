...
// 表層：表面の色
const float3 TopCol = float3(1.0,0.9,0.8);	// 色
const float TopScale = 2.0;					// 視線との角度差に反応する度合い。
const float TopBias = 0.01;					// 正面でどの程度影響を与えるか
const float TopIntensity = 0.2;			// 全体影響度
// 深層：内部の色
const float3 BottomCol = float3(1.0, 0.2, 0.0);	// 色
const float BottomScale = 1.0;			// 視線との角度差に反応する度合い。
...
const float SecondSpecularSmooth =	 0.6;		// 1に近づくほどスペキュラが鋭くなる。(0～1)
const float SecondSpecularIntensity = 0.3;		// スペキュラの強度。0でオフ。1で等倍。(0～)
