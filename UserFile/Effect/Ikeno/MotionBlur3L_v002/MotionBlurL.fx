...
static float3 LastCameraPos = CameraBuffer[0].xyz;
static float3 LastCameraDir = CameraBuffer[1].xyz;
static bool IsSceneChange = 
#if 1
	false; // always false
#else
	(length(CameraPosition - LastCameraPos) > SceneChangeThreshold) ||
	(dot(CameraDirection, LastCameraDir) < SceneChangeDirThreshold) ||
	ChangeCamera;
#endif
//-----------------------------------------------------------------
