...
// 粒子数設定
#define UNIT_COUNT   1   // ←この数×1024 が一度に描画出来る粒子の数になる(整数値で指定すること)
...
#define TEX_FileName  "pattern.png"  // 粒子に貼り付けるテクスチャファイル名
...
float ParticleInitPos = 0.5;       // 粒子発生時の分散位置(大きくすると粒子の初期配置が広くなります)
float ParticleLife = 8.0;          // 粒子の寿命(秒)
float ParticleDecrement = 0.9;     // 粒子が消失を開始する時間(0.0～1.0:ParticleLifeとの比)
float ParticleOccur = 1000.0;         // 粒子発生度(大きくすると粒子が出やすくなる)
float DiffusionAngle = 90.0;       // 発射拡散角(0.0～180.0)
float FloorFadeMax = 1.0;          // フェードアウト開始高さ
float FloorFadeMin = 0.0;          // フェードアウト終了高さ

// 物理パラメータ設定
float3 GravFactor = {0.0, 1.0, 0.0};	// 重力定数
...
// 口元のモーフと泡の発生を関連付けるか?
#define ENABLE_MORPH_EMISSION	0

// 対象モデル名
#define MORPH_TARGET "ここに関連付けるモデルのファイル名を入れる"
// 例： #define MORPH_TARGET "初音ミクmetal.pmd"

// 対象モーフ名
#define MORPH_NAME0		"あ"
#define MORPH_NAME1		"い"
#define MORPH_NAME2		"う"
#define MORPH_NAME3		"え"
#define MORPH_NAME4		"お"
#define MORPH_NAME5		"(予約)"
#define MORPH_NAME6		"(予約)"
#define MORPH_NAME7		"(予約)"
#define MORPH_NAME8		"(予約)"
#define MORPH_NAME9		"(予約)"
...
#define	COORD_TEX_NAME		ParticleCoordTexMouth
