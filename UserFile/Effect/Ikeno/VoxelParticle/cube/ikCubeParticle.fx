...
float4 Particle_PS( VS_OUTPUT2 IN, uniform bool useShadow ) : COLOR0
{
	...
	float3 hitposScaled = hitpos * (2.0 / ParticleSize);
	float3 N = normalize(abs(hitposScaled));
	N = normalize(step(max(N.x, max(N.y, N.z)).xxx - 0.05, N) * sign(localViewDirection));
	float2 Tex = CalcUV(N, hitposScaled);
	N = mul(N, matRot);
...
float4 ZValuePlot_PS( VS_OUTPUT2 IN ) : COLOR0
{
	...
	float tFar  = min(tFar1.x , min(tFar1.y , tFar1.z ));
	// ヒットしない
	// if (tFar < tNear) return float4(0,0.5,1,1);
	clip((tFar > tNear) - 0.1);

	float3 hitpos = localCameraPosition + localViewDirection * tFar;
	float3 wpos = mul(hitpos, matRot) + IN.WPos.xyz;
