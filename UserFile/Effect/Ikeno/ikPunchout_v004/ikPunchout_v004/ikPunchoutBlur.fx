...
#define ENABLE_MASK_BLUR	0 // マスク部分の色を強くボカす
// ※ アップにすると不自然になる
...
DECL_TEXTURE( BlurMap2, BlurSamp2, 2)
...
VS_OUTPUT_BLUR VS_SetTexCoord( float4 Pos : POSITION, float4 Tex : TEXCOORD0, uniform float level)
{
...
float4 PS_Blur(VS_OUTPUT_BLUR IN, uniform sampler2D smp, uniform bool bBlurX) : COLOR
{
	float2 texCoord = IN.TexCoord;
	float2 offset = ((bBlurX) ? float2(IN.Offset.x, 0) : float2(0, IN.Offset.y));
	float4 sum = 0;

	float w = BlurWeight[0];
	float4 col = float4(tex2D(smp, texCoord).rgb, 1);
	#if ENABLE_MASK_BLUR > 0
	float maskBlurRatio = 0.95;
	float mask = tex2D(MaskSampClamp, texCoord).x;
	// col.rgb = lerp(col.rgb, CHANGE_MASK_COLOR, mask);
	w *= saturate(1 - mask * maskBlurRatio);
	#endif
	sum = col * w;

	for(int i = 1; i < 8; i++)
	{
		float4 colp = tex2D(smp, texCoord + offset * i);
		float4 coln = tex2D(smp, texCoord - offset * i);
		float wp = BlurWeight[i];
		float wn = BlurWeight[i];

		#if ENABLE_MASK_BLUR > 0
		float maskp = tex2D(MaskSampClamp, texCoord + offset * i).x;
		float maskn = tex2D(MaskSampClamp, texCoord - offset * i).x;
		// colp.rgb = lerp(colp.rgb, CHANGE_MASK_COLOR, maskp);
		// coln.rgb = lerp(coln.rgb, CHANGE_MASK_COLOR, maskn);
		wp *= saturate(1 - maskp * maskBlurRatio);
		wn *= saturate(1 - maskn * maskBlurRatio);
		#endif

		sum += float4(colp.rgb,1) * wp;
		sum += float4(coln.rgb,1) * wn;
	}
...
float4 DrawBuffer_PS( VS_OUTPUT IN ) : COLOR0
{
	...
	float4 col = tex2D(ScnSamp, IN.Tex);
	float4 blurcol = tex2D(BlurSamp2, IN.Tex);

	return float4(lerp(col.rgb, blurcol.rgb /** MaskColor*/, maskRate), 1);
}
