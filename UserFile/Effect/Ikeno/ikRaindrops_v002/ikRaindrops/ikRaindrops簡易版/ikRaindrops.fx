...
// 落下時の変形
// 横歩行の細まり率
static float ParticleGrowTopWidthScale = 0.5;
static float ParticleGrowBottomWidthScale = 0.8;
// 縦方向の伸び率
static float ParticleGrowHeightScale = 4.0;
...
float4 UpdatePos_PS(float2 Tex: TEXCOORD0) : COLOR
{
		...
		Pos.w == 0.0)
	{
		if (index < AcsTr * TEX_WIDTH && Pos.z < 0.5)
		{
			...
			Pos.z -= elapseTime;
			if (Pos.z < 0.0) Pos.xyz = RAND(index, 1.0, 30.0).xyz;
		}
	}
	else if (Pos.w > 0.0)
	{
...
VS_OUTPUT2 Particle_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0)
{
	...
	if (particle.z < 0.0)
	{
		...
		if (Pos.y > 0.0)
		{
			...
			Pos.x *= (1 - (1 - ParticleGrowTopWidthScale) * rate);
			thick = 1.0 - 0.5 * rate;
		}
		else
		{
			Pos.x *= (1 - (1 - ParticleGrowBottomWidthScale) * rate);
		}
		thick *= (1.0 - 0.5 * rate);
	}
...
float4 Particle_PS( VS_OUTPUT2 IN ) : COLOR0
{
	...
	c.rgb += saturate(pow( max(0,dot( HalfVector, mul( N, WorldMatrix ))), SpecularPower)) * SpecularScale;
