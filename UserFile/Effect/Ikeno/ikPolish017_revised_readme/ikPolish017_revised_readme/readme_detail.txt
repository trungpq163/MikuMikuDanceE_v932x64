...
v.0.16, v.0.17で追加した項目については[New!]のマークをつけてあります。
...
　※ HDRから変換したスカイドームを使用している場合は、TEnvMap_rgbm.fxを割り当ててください。[New!]
...
　[New!] ※ Ver.0.17からスペキュラカラーを参照しないモードを追加しました。
...
　[New!] USE_ALBEDO_AS_SPECULAR_COLOR
　　ベース色を反射色として使用するかどうか。
...
　　[New!] きしめんステージのスポットライトモデルのように影を落とさないモデルは
　　除外しておいたほうが多少高速になる。

　[New!] SSAOMapRT
　　SSAO用の深度を設定する。
　　SSAO_QUALITYが1以下の場合には存在しない。
...
　[New!] MMDの表示 > アンチエイリアスのチェックを切ってみる。
　[New!] MMEのエフェクト割当で、ikPolishの使用する ColorMapRT, ENvMapRT, 
　　RSMMapRT, SSAOMapRT, LightDepthRTタブから、不要そうなものを外す。
　[New!] 32bit版を使用している場合、64bit版を使ってみる。
...
　　　 [New!] ikPolishController.pmxを入れて、"シャドウ"モーフで影を柔らかくする。
...
　　　[New!] マテリアルファイルで、USE_ALBEDO_AS_SPECULAR_COLOR	を1にしてみる。
