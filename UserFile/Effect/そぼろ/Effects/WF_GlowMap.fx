...
//発光マップテクスチャ名
#define TEXTURE_NAME "filename.png"
...
float4 TransMirrorPos( float4 Pos ){
    float4x4 mirrorMat = float4x4(
       1.0f-2.0f*WldMirrorNormal.x*WldMirrorNormal.x,     -2.0f*WldMirrorNormal.x*WldMirrorNormal.y,     -2.0f*WldMirrorNormal.x*WldMirrorNormal.z, 0.0f,
           -2.0f*WldMirrorNormal.x*WldMirrorNormal.y, 1.0f-2.0f*WldMirrorNormal.y*WldMirrorNormal.y,     -2.0f*WldMirrorNormal.y*WldMirrorNormal.z, 0.0f,
           -2.0f*WldMirrorNormal.x*WldMirrorNormal.z,     -2.0f*WldMirrorNormal.y*WldMirrorNormal.z, 1.0f-2.0f*WldMirrorNormal.z*WldMirrorNormal.z, 0.0f,
                                        WldMirrorPos,       1.0f );
    Pos = float4(Pos.xyz - WldMirrorPos, 1.0f);
    Pos = mul( Pos, mirrorMat ); // 鏡像変換
    return Pos;
}
...
float IsFace( float4 pos ){
    float ans = 1.0f;
    if(dot(pos.xyz-WldMirrorPos, WldMirrorNormal) < 0.0f ) ans = 0.0f;
    if(dot(CameraPosition-WldMirrorPos, WldMirrorNormal) < 0.0f) ans = 0.0f;
    return ans;
}
...
struct VS_OUTPUT {
    ...
    float4 WPos       : TEXCOORD2;   // 鏡像元のワールド座標
};
...
VS_OUTPUT Basic_VS(MMM_SKINNING_INPUT IN)
{
    ...
    Out.WPos = mul( pos, WorldMatrix ); // ワールド座標
