...
//前フレームのワールドビュー射影行列
static float4x4 lastMatrixL = float4x4(MatrixBufArray[0], MatrixBufArray[1], MatrixBufArray[2], MatrixBufArray[3]);
//

#ifdef MIKUMIKUMOVING
    static float4x4 lastMatrix = mul(WorldMatrix, lastMatrixL);
#else
    static float4x4 lastMatrix = lastMatrixL;
#endif
...
float4 DrawMatrixBuf_PS(float2 texCoord: TEXCOORD0) : COLOR {
    ...
    if(dindex < 4){
        #ifdef MIKUMIKUMOVING
            Color = ViewProjMatrix[(int)dindex]; //行列を記録 ※MMMではBuffer描画でWorldViewProjMatrix使用不可っぽい
        #else
