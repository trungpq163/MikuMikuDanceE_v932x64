...
float ftime2 : TIME <bool SyncInEditMode=false;>;
...
//   ftime : フレーム時間です。単位は秒です。
//   ftime2 : フレーム時間です。単位は秒です。編集中も進み続けます。
...
float4 ClonePos(float4 Pos : POSITION) : POSITION 
{
    ...
    float cindex = index - center;
    
    float column = (int)(index / row_count);    //行番号
    float row = ((index % row_count) - center); //列番号
    ...
    Pos = mul(Pos, inverseDir(MasterWorldMat));
...
// スクリーンサイズ
static float2 ViewportOffset = (float2(0.5,0.5)/MIPMAPTEX_SIZE);
