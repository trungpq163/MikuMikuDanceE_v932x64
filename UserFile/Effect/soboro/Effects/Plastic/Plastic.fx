...
static float3 SpecularColor = lerp(MaterialSpecular, float3(1,1,1), 0.2) * LightSpecular;
...
VS_OUTPUT Basic_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
    ...
    if ( !useToon ) {
        //float3 dcolor = lerp(DiffuseColor.rgb, LightAmbient, 0.3);
        //float3 dcolor = lerp(DiffuseColor.rgb, LightSpecular / 0.6, 0.5);
        float3 dcolor = DiffuseColor.rgb  * (LightSpecular + 0.4);
        //float3 dcolor = DiffuseColor.rgb;
        Out.Color.rgb += max(0,dot( Out.Normal, -LightDirection )) * dcolor;
    }
...
{
    float sppow = SpecularPower + 5;
    
    // スペキュラ色計算
    float3 HalfVector = normalize( normalize(IN.Eye) + -LightDirection );
    float SpecularRatio = pow( max(0,dot( HalfVector, normalize(IN.Normal) )), sppow );
    
    float3 HalfVector2 = normalize( normalize(IN.Eye) + LightDirection );
    float SpecularRatio2 = pow( max(0,dot( HalfVector2, normalize(IN.Normal) )), sppow );
    
    float3 HalfVector3 = normalize(IN.Eye);
    float SpecularRatio3 = pow( max(0,dot( HalfVector3, normalize(IN.Normal) )), sppow );
    
    SpecularRatio = saturate((SpecularRatio - 0.6) * 5) * 1.2;
    SpecularRatio2 = saturate((SpecularRatio2 * 0.8 - 0.5) * 1);
    SpecularRatio2 += saturate((SpecularRatio3 * 0.8 - 0.6) * 1);
    
    float3 SpecularBase = saturate((SpecularColor + 0.3) * 0.8);
    float3 Specular = SpecularRatio * SpecularBase;
    float3 Specular2 = SpecularRatio2 * SpecularBase;
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    if( any( saturate(TransTexCoord) != TransTexCoord ) ) {
        ...
        if ( useToon ) {
            ...
            float LightNormal = dot( IN.Normal, -LightDirection );
            comp = min(saturate(LightNormal * 1 + 0.5),comp);
            ShadowColor.rgb *= MaterialToon;
        }
    ...
    Color.rgb -= saturate(Color.rgb - 1.0) * 0.4;
