...
static float4 FogColor = float4(0.3f, 0.3f, 0.3f, 1.0f);    // 霧の色(RGBA)
static float FogStart = 100.0f * FogSize, FogDistance = 3000.0f * FogSize;    // 霧のかかる距離、範囲
...
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR0
{
    ...
    float CoverAlpha = saturate(CoverNormal * 1.5 + 0.2) * 0.5; //合成度を決定
    Color.rgb = lerp(Color.rgb, CoverColor1, CoverAlpha); //色合成
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    // Rain Color
    float CoverNormal = dot( IN.Normal, float3(0,1,0)); //法線とY軸ベクトルの内積
    float CoverAlpha = saturate(CoverNormal * 1.5 + 0.2) * 0.6; //合成度を決定
    Color.rgb = lerp(Color.rgb, CoverColor1, CoverAlpha); //色合成
    ShadowColor.rgb = lerp(ShadowColor.rgb, CoverColor1, CoverAlpha); //色合成
