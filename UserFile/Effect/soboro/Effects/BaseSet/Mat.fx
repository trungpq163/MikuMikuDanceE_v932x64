...
VS_OUTPUT Basic_VS(float4 Pos : POSITION, float3 Normal : NORMAL, float2 Tex : TEXCOORD0, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon)
{
    ...
    Out.Color.rgb = saturate(MaterialAmbient * LightSpecular + MaterialEmmisive);
    if ( !useToon ) {
        Out.Color.rgb += DiffuseColor.rgb * 0.5;
    }
...
float4 Basic_PS(VS_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR0
{
    ...
    if ( useToon ) {
        ...
        Color.rgb *= lerp(MaterialToon * 0.95, float3(1,1,1), saturate(LightNormal * 1 + 0.5));
    }else{
        float LightNormal = dot( IN.Normal, -LightDirection );
        Color.rgb *= lerp(0.5, 1, saturate(LightNormal * 1 + 0.5));
    }
